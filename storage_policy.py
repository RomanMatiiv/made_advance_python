...
class ZlibStoragePolicy(StoragePolicy):
    """
    Сохранение и извлечение инвертированного индекса на диск c сжатием
    """
...
class StructStoragePolicy(StoragePolicy):
    ...
    Работа с инвертированным индексом происходит следующим образом

    Вся информация делится на 2 части
    1 формат в котором информацию нужно сохранить
      (см модуль struct: https://tirinox.ru/python-struct/)
    2 информация
        2.1 слово
        2.2 документы в которых это слово встречается

    Формат должен быть строкой из 2 логических частей
        1) формат для слова(2.1) Пример "10s" - слово из 10 букв
        2) формат для документов (2.2) Пример "3H" массив из 3 документов [1,2,3]
    Итого формат "10s 3H"

    Тк сам формат тоже нужно записать на диск то он тоже кодироуется числама
    Пример "10s 3H" -> [101, 32]
        где кодировка s -> 1 H -> 2 придумывалась самостоятельно

    Общий пример
        "2I" - маска для метаинформации

        Сохранение (dump)
            1 - получение маски из информации(информация) -> "10s 3H"
            2 - "10s 3H" -> [101, 32]
            3 - сохранение мета информации на диск("2I", [101, 32])
            4 - сохранение информации на диск("10s 3H", информация)

        Извлечение (load)
            1 - извлечение маски для информации("2I", raw_data) -> [101, 32]
            2 - получение маски для информации [101, 32] -> "10s 3H"
            3 - извлечение информации("10s 3H", raw_data)
